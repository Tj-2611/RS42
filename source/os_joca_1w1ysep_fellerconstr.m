function [optV_all, rmseV_all, optmdl_all, rmsemdl_all] = os_joca_1w1ysep_fellerconstr(year, hy, data_path, save_path, analysis_model, method)
% out-of-sample test by calibrating spot variances with fixed structural
% parameters that satisfying the Feller condition
currentDir = pwd;
parentDir = fileparts(currentDir);
parafilepath = [parentDir, '/estimates/'];
model = analysis_model;
load([parafilepath, '/', num2str(year-1), '_', analysis_model, '_Fellerconstr.mat'],'optmdl_all');
...
parfor t_index = 1:t_num
  ...
  errV1 = @(x)(cal_volsurfsepos([x; paramdl_tmp], spx_tmp, vix_tmp, model, method, 'ip'));
  [V_tmp1,fval1] = fmincon(errV1,[V0],[],[],[],[],[V_bd(:, 1)],[V_bd(:, 2)],[], options_tmp1);
  errV2 = @(x)(cal_volsurfsepos([x; paramdl_tmp], spx_tmp, vix_tmp, model, method, 'lm'));
